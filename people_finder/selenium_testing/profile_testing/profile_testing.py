...
try:
    ...
    dp_upload.send_keys(r'E:\prog\uap-people-finder\people_finder\selenium_testing\azim_islam_dp.jpg')
...
try:
    first_name = driver.find_element(By.XPATH, '//*[@id="id_first_name"]')
    assert first_name.get_attribute('value') == fname, "Test: First name does not match ❌"
    print("Test: First name matches ✅")
except AssertionError as msg:
    print(msg)
try:
    last_name = driver.find_element(By.XPATH, '//*[@id="id_last_name"]')
    assert last_name.get_attribute('value') == lname, "Test: Last name does not match ❌"
...
try:
    ...
    assert 'azim_islam_dp.jpg' in dp.get_attribute('src'), "Test: Display Picture does not match with (azim_islam_dp.jpg) ❌"
    print( "Test: Display Picture matches with (azim_islam_dp.jpg) ✅")
except AssertionError as msg:
    print(msg)
try:
    dp = driver.find_element(By.XPATH, '/html/body/div/div/img')
    assert 'busra_dp.jpg' in dp.get_attribute('src'), "Test: Display Picture does not match with (busra_dp.jpg) ❌"
